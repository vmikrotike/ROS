...
def patch_npk_package(package, key_dict):
    if package[NpkPartID.NAME_INFO].data.name == 'system':
        ...
        run_shell_command(f"sudo sed -i '1s#.*#  MMM      MMM       KKK                           TTTTTTTTTTT       KKK#' {logo}")
        run_shell_command(f"sudo sed -i '2s#.*#  MMMM    MMMM  III  KKK  KKK  RRRRRR     OOOOOO       TTT      III  KKK  KKK#' {logo}")
        run_shell_command(f"sudo sed -i '3s#.*#  MMM MMMM MMM  III  KKKKK     RRR  RRR  OOO  OOO      TTT      III  KKKKK#' {logo}")
        run_shell_command(f"sudo sed -i '4s#.*#  MMM  MM  MMM  III  KKK KKK   RRRRRR    OOO  OOO      TTT      III  KKK KKK#' {logo}")
        run_shell_command(f"sudo sed -i '5s#.*#  MMM      MMM  III  KKK  KKK  RRR  RRR   OOOOOO       TTT      III  KKK  KKK#' {logo}")
        #run_shell_command(f"sudo sed -i '8s#.*#  MikroTik RouterOS 7.17 Free Version (Non-Commercial)#' {logo}")
        run_shell_command(f"sudo sed -i '6s#.*#  #' {logo}")
